...
class CarEvaluation:
    ...
    def __init__(self, name, price, safety):
        self.name,self.price,self.safety,CarEvaluation.carCount = name, price, safety, CarEvaluation.carCount + 1
...
def sortbyprice(listCarEvaluation, order):  #you fill in the rest
    return sorted(L,key = lambda x: {"High":3,"Med":2,"Low":1}[x.price],reverse={"asc":False,"des":True}[order]) #return a value
